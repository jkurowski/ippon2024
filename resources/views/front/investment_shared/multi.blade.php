    ...
    <div class="container-fluid">
        @if($buildings->count() > 0)
            @foreach($buildings as $building)
                <h3 class="building-title mt-5 mb-4">{{ $building->name }}</h3> {{-- Building name as a section title --}}
                        ...
                        <div class="row property-list-item">
                                            ...
                                            <div class="col-12 @if($room->additional) col-lg-6 @endif">
                                                <div class="row">
                                                    <div class="col-12 col-sm-4 property-list-item-stat mb-3 mb-lg-0">
                                                        <img src="{{ asset('/images/floor-icon.svg') }}" alt="Ikonka piętra" class="me-3">
                                                        @lang('website.select-option-floor') {{ isset($room->floor_number) ? $room->floor_number : $room->floor->number }}
                                                    </div>
                                                    <div class="col-12 col-sm-4 property-list-item-stat mb-3 mb-lg-0">
                                                        <img src="{{ asset('/images/room-icon.svg') }}" alt="Ikonka pokoi" class="me-3">
                                                        {{$room->rooms}}
                                                        @if ($room->rooms == 1)
                                                            @lang('website.room')
                                                        @else
                                                    ...
                                                    <div class="col-12 col-sm-4 property-list-item-stat">
                                                        <img src="{{ asset('/images/area-icon.svg') }}" alt="Ikonka powierzchni" class="me-3">
                            ...
                            <div class="col-12 col-lg-2 d-flex align-items-center">
                                ...
                                @if(Route::currentRouteName() === 'clipboard.index')
                                    <button id="addToFav" class="bttn mt-3" data-id="{{$room->id}}">
                                ...
                                @else
                                    <a href="{{ route('developro.property', [$investment->slug, $room, Str::slug($room->name), floorLevel($room->floor_number, true), number2RoomsName($room->rooms, true), round(floatval($room->area), 2).'-m2']) }}"
                                       class="bttn bttn-icon">
                                        @lang('website.show-room') <i class="ms-4 las la-file"></i>
                                    </a>
                                @endif
                            </div>
                ...
                @else
                    <div class="row">
                        <div class="col-12 text-center">
                            <b>Brak dostępnych mieszkań w tym budynku</b>
                        </div>
                    </div>
                @endif
            @endforeach
