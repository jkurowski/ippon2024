                                ...
                                @if($properties)
                                    @foreach($properties as $r)
                                        @if($r->html)
                                            <area
                                                    shape="poly"
                                                    href="@if($r->status <> 3) {{ route('developro.property', [$investment->slug, $r, Str::slug($r->name), floorLevel($r->floor_number, true), number2RoomsName($r->rooms, true), round(floatval($r->area), 2).'-m2']) }} @else # @endif"
                                                    data-item="{{$r->id}}"
